...
export async function runMigrations(): Promise<string[]> {
  ...
  for (const file of files) {
    ...
    try {
      ...
      throw new Error(`[db] migration ${file} failed: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
      client.release();
    }
  }
...
export async function initDb(): Promise<boolean> {
  try {
    ...
    console.log(`[db] connected: ${version?.split(',')[0]}${ran.length ? ` — applied ${ran.length} migration(s): ${ran.join(', ')}` : ' — schema up to date'}`);
    return true;
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error(`[db] FAILED to connect or migrate — DB-backed features are disabled until this is fixed: ${msg}`);
    console.error('[db] configure via DATABASE_URL or PGHOST/PGPORT/PGUSER/PGPASSWORD/PGDATABASE (default database: aether_db)');
    return false;
  }
}
...
export function dbExec(sql: string, params: unknown[] = []): Promise<pg.QueryResult | null> {
  return pool.query(sql, params).catch((err) => {
    console.error(`[db] write failed: ${err instanceof Error ? err.message : String(err)} — sql: ${sql.slice(0, 120)}`);
    return null;
  });
}
...
export async function dbRows<T = Record<string, unknown>>(sql: string, params: unknown[] = []): Promise<T[]> {
  try {
    ...
    console.error(`[db] read failed: ${err instanceof Error ? err.message : String(err)} — sql: ${sql.slice(0, 120)}`);
    return [];
  }
}
