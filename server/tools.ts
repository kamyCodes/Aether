...
export interface ToolDef {
  ...
  permission?: {
    ...
    derive: (args: Record<string, unknown>) => { title: string; detail: string; command?: string; path?: string };
  };
...
export class ToolRegistry {
  ...
  jsonSchema() {
    return [...this.tools.values()].map((t) => ({
      ...
      function: { name: t.name, description: t.description, parameters: { type: 'object', properties: t.parameters, required: Object.keys(t.parameters).filter((k) => t.parameters[k].type !== 'optional') } },
    }));
  }
  ...
  private async act(tool: ToolDef, args: Record<string, unknown>, ctx: ToolContext, label: string): Promise<string> {
    // permission gate
    if (tool.permission && !ctx.autoApprove) {
      const d = tool.permission.derive(args);
      const decision = await this.perms.request({ kind: tool.permission.kind, taskId: ctx.taskId, ...d });
    ...
    try {
      ...
      emitActivity({ ...item, status: 'done', icon: '✓', result: result.slice(0, 400), detail: `${Date.now() - started}ms` });
      return result;
    } catch (err) {
  ...
  private registerDefaults() {
    ...
    const withCtx = (fn: (args: Record<string, unknown>, wsId: string) => Promise<string>): ToolDef['execute'] =>
      (args, ctx) => {
        currentCtx = ctx;
    ...
    this.register({
      ...
      permission: {
        kind: 'read',
        derive: (a) => ({ title: `Read file ${a.path}`, detail: 'The agent wants to read a file.', path: String(a.path) }),
      },
    ...
    this.register({
      name: 'write_file',
      description: 'Create or overwrite a file with full content. Produces a diff proposal the user can accept or reject.',
      parameters: { path: { type: 'string', description: 'workspace-relative path' }, content: { type: 'string', description: 'full file content' } },
      permission: {
        kind: 'write',
        derive: (a) => ({ title: `Write file ${a.path}`, detail: 'The agent wants to create or overwrite a file.', path: String(a.path) }),
      },
      execute: withCtx(async (a, wsId) => {
        const p = String(a.path);
        const content = String(a.content ?? '');
        let old = '';
        try { old = await this.ws.readFile(wsId, p); } catch { /* new file */ }
    ...
    this.register({
      ...
      permission: {
        kind: 'write',
        derive: (a) => ({ title: `Edit file ${a.path}`, detail: `Replace "${String(a.old_string).slice(0, 80)}" with new content.`, path: String(a.path) }),
      },
      execute: withCtx(async (a, wsId) => {
        const p = String(a.path);
        ...
        if (!content.includes(oldS)) return `ERROR: old_string not found in ${p}. Read the file again for exact text.`;
    ...
    this.register({
      ...
      permission: {
        kind: 'delete',
        derive: (a) => ({ title: `Delete ${a.path}`, detail: 'This operation cannot be undone except via checkpoints/git.', path: String(a.path) }),
      },
      execute: withCtx(async (a, wsId) => {
    ...
    this.register({
      ...
      execute: withCtx(async (a) => {
        ...
        return syms.length ? syms.map((s) => `${s.kind} ${s.name} @ ${s.path}:${s.line}`).join('\n') : 'No symbols found.';
      }),
    });

    this.register({
      name: 'run_command',
      description: 'Run a shell command in the workspace. Dangerous commands require user permission.',
      parameters: { command: { type: 'string', description: 'shell command' } },
      permission: {
        kind: 'command',
        derive: (a) => ({ title: 'Run terminal command', detail: 'The agent wants to execute a shell command.', command: String(a.command) }),
      },
      execute: withCtx(async (a, wsId) => {
    ...
    this.register({
      ...
      parameters: { name: { type: 'string', description: 'package name' }, dev: { type: 'string', description: '"true" for devDependency' } },
      permission: {
        kind: 'install',
        derive: (a) => ({ title: `Install dependency ${a.name}`, detail: 'Package installation can run arbitrary postinstall scripts.', command: `install ${a.name}` }),
      },
      execute: withCtx(async (a, wsId) => {
        const cwd = this.ws.require(wsId).root;
    ...
    this.register({
      ...
      execute: withCtx(async (_a, wsId) => {
        ...
        const cmd = has('pytest.ini') || has('pyproject.toml') ? 'pytest -q' : has('go.mod') ? 'go test ./...' : has('Cargo.toml') ? 'cargo test -q' : 'npm test -- --run 2>&1 || npm test';
        const r = await this.terms.exec(cmd, cwd, 300_000, ctxSignal());
    ...
    this.register({
      ...
      execute: withCtx(async (_a, wsId) => {
        const cwd = this.ws.require(wsId).root;
        const r = await this.terms.exec('npm run build 2>&1 || make 2>&1', cwd, 300_000, ctxSignal());
        return `exit=${r.code}\n${(r.stdout + r.stderr).slice(0, 8000)}`;
      }),
    });
    ...
    // --- Git tools ---
    const gperm = (a: Record<string, unknown>, title: string) => ({ kind: 'git' as const, derive: () => ({ title, detail: 'Git operation requested by the agent.', command: String(a.command ?? title) }) });
    ...
    this.register({
      ...
      execute: withCtx(async (a, wsId) => {
        ...
        return diffs.map((d) => `${d.path}: +${d.additions} -${d.deletions}`).join('\n') || 'No changes.';
      }),
    });
    ...
    this.register({
      ...
      permission: {
        kind: 'git',
        derive: (a) => ({ title: 'Create git commit', detail: `Message: ${a.message}`, command: `git commit -m "${a.message}"` }),
      },
      execute: withCtx(async (a, wsId) => {
        const wsRoot = this.ws.require(wsId).root;
        const s = await this.gitm.status(wsRoot);
        await this.gitm.addAll(wsRoot, s.files.map((f) => f.path));
    ...
    this.register({
      ...
      permission: {
        kind: 'git',
        derive: (a) => ({ title: `Create branch ${a.name}`, detail: 'Creates and checks out a new branch.', command: `git checkout -b ${a.name}` }),
      },
      execute: withCtx(async (a, wsId) => {
    ...
    this.register({
      name: 'save_memory',
      description: 'Save a durable fact, decision, or convention about this project to long-term memory. Use for things future sessions must know: user preferences, architecture decisions, gotchas, workflow rules. Not for task-specific details.',
    ...
    this.register({
      ...
      permission: {
        kind: 'write',
        derive: (a) => ({ title: `Restore checkpoint ${a.id}`, detail: 'Files will be reverted to the checkpoint state.', command: `restore ${a.id}` }),
      },
      execute: withCtx(async (a, wsId) => {
    ...
    // context helpers
    function ctxTask() { return currentCtx?.taskId ?? 'adhoc'; }
    function ctxSignal() { return currentCtx?.signal; }
  }
  ...
  async createCheckpointFor(wsId: string, taskId: string, label: string) {
    ...
    for (const p of files.slice(0, 300)) {
      try {
        const stat = await (await import('node:fs/promises')).stat(path.join(this.ws.require(wsId).root, p));
        if (stat.size > 200_000) { snapshot.push({ path: p, content: null }); continue; }
        snapshot.push({ path: p, content: await this.ws.readFile(wsId, p) });
      } catch { snapshot.push({ path: p, content: null }); }
    }
...
function humanLabel(tool: string, args: Record<string, unknown>): string {
  switch (tool) {
    ...
    case 'list_files': return 'Listed project files';
    case 'search_files': return `Searched "${args.query}"`;
    case 'search_symbols': return `Searched symbols "${args.query}"`;
    case 'run_command': return `Running \`${args.command}\``;
    case 'install_dependency': return `Installing ${args.name}`;
    case 'run_tests': return 'Running tests…';
    case 'run_build': return 'Running build…';
    case 'inspect_errors': return 'Inspecting errors';
    case 'git_status': return 'Git status';
    case 'git_diff': return 'Git diff';
    case 'git_commit': return `Commit: ${args.message}`;
    case 'git_branch': return `Branch: ${args.name}`;
    case 'create_checkpoint': return `Checkpoint: ${args.label}`;
    case 'restore_checkpoint': return `Restoring checkpoint ${args.id}`;
    default: return tool;
  }
}
