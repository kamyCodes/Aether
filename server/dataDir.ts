/**
 * DATA_DIR resolution — one place decides where per-user app data lives
 * (respects AETHER_HOME); every other module derives paths from it.
 */
...
if (fs.existsSync(LEGACY_DIR)) {
  try {
    ...
    for (const entry of fs.readdirSync(LEGACY_DIR)) {
      ...
      const needsCopy =
        !fs.existsSync(dst) || (fs.statSync(dst).isFile() && fs.statSync(dst).size === 0); // e.g. interrupted earlier migration
