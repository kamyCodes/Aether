      ...
      {tabs.map((t) => {
        ...
        return (
          ...
          >
            ...
            <span>
              {t.title}
              {t.preview && <Eye size={10} style={{ marginLeft: 4 }} />}
            </span>
            {!t.pinned && (
              <span
                className="close"
...
function tabIcon(kind: string, title: string): React.ReactNode {
  switch (kind) {
    case 'diff':
      return <GitCompare size={12} className="icon-run" />;
    case 'git':
      return <GitBranch size={12} />;
    case 'review':
      return <GitPullRequestArrow size={12} className="icon-warn" />;
    case 'search':
      return <Search size={12} />;
    case 'skills':
      return <Sparkles size={12} className="icon-warn" />;
    case 'map':
    ...
    default: {
      ...
      return (
        <img
          ...
          style={{ flexShrink: 0 }}
          onError={(e) => {
            e.currentTarget.src = '/icons/vscode/default_file.svg';
          }}
        />
      );
    }
