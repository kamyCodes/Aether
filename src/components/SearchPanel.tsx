...
export function SearchPanel() {
  ...
  async function run() {
    ...
    setBusy(true);
    try {
      setResp(await post<SearchResp>(`/workspaces/${workspaceId}/search`, { query }));
    } finally {
      setBusy(false);
    }
  }
        ...
        <div>
          ...
          {resp.symbols.map((s, i) => (
            <div key={i} className="result-item" onClick={() => void openFile(s.path)}>
              <span className="badge">{s.kind}</span> {s.name}{' '}
              <span className="hint">
                {s.path}:{s.line}
              </span>
            </div>
          ...
          {resp.files.map((f) => (
            <div key={f.path} className="result-item" onClick={() => void openFile(f.path)}>
              <span className="file-ico">
                <FileIcon name={f.path.split('/').pop() ?? f.path} size={14} />
              </span>
              <span style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>{f.path}</span>
              <span className="hint">score {f.score.toFixed(1)}</span>
            </div>
          ))}
          {resp.files.length === 0 && resp.symbols.length === 0 && (
            <div className="empty-state">No results (index may still be building).</div>
          )}
        </div>
      )}
    </div>
