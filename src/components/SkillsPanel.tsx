...
export function SkillsPanel() {
  ...
  const importPack = async (overwrite: boolean) => {
    ...
    try {
      ...
      setImportNote(
        `Imported ${body.imported} skill${body.imported === 1 ? '' : 's'}${body.skipped ? ` · ${body.skipped} already installed` : ''}`,
      );
  ...
  const importSkill = () => {
    ...
    input.onchange = async () => {
      ...
      if (file) {
        try {
          await saveSkill(JSON.parse(await file.text()) as Skill);
        } catch (e) {
          void alertDialog('Invalid skill file', e instanceof Error ? e.message : String(e));
        }
      }
    };
    input.click();
      ...
      >
        <div className="panel-header">
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            <button onClick={importSkill} title="Import a .skill.json file">
              <FilePlus2 size={12} /> Import
            </button>
            <button
              onClick={() => void importPack(false)}
              disabled={importing}
              title="Install the 75-skill Aether skill pack (existing installs untouched)"
            >
        ...
        <div className="skills-toolbar">
          ...
          <div className="mode-toggle" aria-label="Skill category">
            ...
            <div className="segment-indicator">
              <div className="segment-indicator-fill" />
            </div>
          </div>
          ...
          {visible.map((s) => (
            <div key={s.id} className="skill-card">
              <div className="head">
                ...
                <div style={{ flex: 1 }} />
                <button onClick={() => setEditing(s)} title="Edit skill">
                  <Pencil size={11} />
                </button>
                <button onClick={() => exportSkill(s)} title="Export as .skill.json">
                  <Download size={11} />
                </button>
                {!s.builtin && (
                  <button
              ...
              {s.enabled && (
                <div
                  className="skill-invocation-hint"
                  title="Type this in the chat composer to invoke the skill"
                >
      ...
      <div className="skill-editor-body">
        <label className="se-field">
          ...
          <input
            ...
            placeholder="e.g. TypeScript Strictness"
          />
        </label>
        ...
        <label className="se-field">
          ...
          <textarea
            ...
            placeholder="One line the agent sees when deciding whether the skill applies"
          />
        </label>

        <label className="se-field">
          <span>
            Instructions <em>— system-prompt fragment</em>
          </span>
          <textarea
            rows={8}
            value={skill.instructions}
            onChange={(e) => set({ instructions: e.target.value })}
            placeholder="What the agent should do when this skill is active…"
          />
        </label>

        <label className="se-field">
          <span>
            Rules <em>— one per line</em>
          </span>
          <textarea
            rows={5}
            value={skill.rules.join('\n')}
            onChange={(e) => set({ rules: e.target.value.split('\n').filter(Boolean) })}
            placeholder={'Always run tests after edits\nNever modify lockfiles'}
          />
        </label>

        <label className="se-field">
          <span>
            Examples <em>— one per line</em>
          </span>
          <textarea
            rows={3}
            value={(skill.examples ?? []).join('\n')}
            onChange={(e) => set({ examples: e.target.value.split('\n').filter(Boolean) })}
            placeholder="Concrete good examples the agent can imitate"
          />
        </label>
        ...
        <div className="se-row">
          ...
          <label className="se-field">
            <span>
              Priority <em>— higher wins context</em>
            </span>
            <input
              type="number"
