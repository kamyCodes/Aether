...
function StatusIcon({ status }: { status: string }) {
  switch (status) {
    case 'running':
    case 'planning':
      return <Loader2 size={12} className="icon-run spin" />;
    case 'done':
    case 'completed':
      return <Check size={12} strokeWidth={2.5} className="icon-ok" />;
    case 'error':
    case 'failed':
      return <X size={12} strokeWidth={2.5} className="icon-err" />;
    case 'cancelled':
      return <Ban size={12} className="icon-dim" />;
    case 'paused':
    case 'awaiting_permission':
    case 'awaiting_question':
      return <Pause size={12} className="icon-warn" />;
    default:
      return <Circle size={10} className="icon-dim" />;
  }
}
...
function ToolCard({ a }: { a: ActivityItem }) {
  ...
  return (
    <div className={`ac-tool ${state} fade-in`}>
      <button
        className="ac-tool-head"
        onClick={() => hasResult && setOpen((o) => !o)}
        title={hasResult ? 'Toggle details' : undefined}
      >
        ...
        {(a.filePath || a.command) && (
          <span className="ac-tool-param mono-path">{a.filePath ?? a.command}</span>
        )}
        <span className="ac-tool-status">
      ...
      {open && hasResult && (
        <pre className={`ac-tool-result ${a.status === 'error' ? 'error' : ''}`}>
          {a.error ?? a.result ?? a.detail}
        </pre>
      )}
    </div>
  );
    ...
    <div className={`ac-actions ${open ? 'open' : ''} ${finished ? 'finished' : ''} fade-in`}>
      <button className="ac-actions-head" onClick={() => setOpen((o) => !o)} aria-expanded={open}>
        <span className="ac-actions-icon">
          {running ? (
            <Loader2 size={12} className="icon-run spin" />
          ) : finished ? (
        ...
        <span className="ac-actions-count">
          {done}/{items.length}
        </span>
        {errored > 0 && <span className="ac-actions-errors">{errored} failed</span>}
        {current && !finished && (
          <span className="ac-tool-param mono-path">
            {current.tool ?? current.command ?? 'working…'}
          </span>
        )}
        <span className="tool-expand">
          {open ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
    ...
    <div className="ac-prompt fade-in">
      ...
      <div className="choice-row">
        {p.decision ? (
          ...
          <>
            <button className="primary" onClick={() => void decidePermission(p.id, 'allow')}>
              Allow Once
            </button>
            <button onClick={() => void decidePermission(p.id, 'always')}>Allow Always</button>
            <button className="danger" onClick={() => void decidePermission(p.id, 'deny')}>
              Deny
            </button>
          </>
        )}
...
function QuestionInline({
  ...
  q: { id: string; title: string; kind: string; options?: { label: string; value: string }[] };
}) {
  ...
  if (answered) {
    return (
      <div className="ac-prompt fade-in">
        ...
        <div className="choice-row">
          <span className="choice-made">Answered: {answered}</span>
        </div>
      </div>
    );
  }
    ...
    <div className="ac-prompt fade-in">
      ...
      {q.kind === 'text' && (
        <div className="choice-row">
          ...
          />
          <button className="primary" disabled={!text.trim()} onClick={() => answer(text.trim())}>
            Continue
          </button>
        </div>
      )}
      ...
      {q.kind === 'confirm' && (
        <div className="choice-row">
          <button className="primary" onClick={() => answer('confirm')}>
            Continue
          </button>
  ...
  return (
    <div className="ac-tasklist fade-in">
      <div className="ac-tasklist-head">
        ...
        <span className="ac-tasklist-track">
...
function DiffArtifactCard({ diff }: { diff: DiffArtifact }) {
  const [open, setOpen] = useState(false);
  const statusLabel =
    diff.status === 'added' ? 'new' : diff.status === 'deleted' ? 'deleted' : 'edited';
  return (
...
function InlineMonacoDiff({ original, modified }: { original: string; modified: string }) {
  return (
    <MonacoDiff
      ...
      height={Math.min(
        360,
        Math.max(120, Math.max(original.split('\n').length, modified.split('\n').length) * 19 + 20),
      )}
    />
  );
}
          ...
          <>
            ...
            {w.verification.map((v, i) => (
              <div key={i} className={`ac-verify ${v.ok ? 'ok' : 'fail'}`}>
                <button
                  className="ac-verify-head"
                  onClick={() => setOpenIdx(openIdx === i ? null : i)}
                >
                  {v.ok ? (
                    <Check size={12} className="icon-ok" />
                  ) : (
                    <X size={12} className="icon-err" />
                  )}
...
function PlanArtifact({
  ...
  task: { id: string; plan: NonNullable<import('../../shared/types').AgentTask['plan']> };
}) {
  ...
  return (
    <div className="ac-plan fade-in">
      ...
      <div
        className="ac-plan-body"
        onClick={(e) => {
          const lineEl = (e.target as HTMLElement).closest('[data-line]');
          if (lineEl) {
            setPendingComment(Number(lineEl.getAttribute('data-line')));
            setCommentText('');
          }
        }}
      >
        {lines.map((ln, i) => (
          <div
            key={i}
            data-line={i}
            className={`ac-plan-line ${pendingComment === i ? 'commenting' : ''} ${plan.comments.some((c) => c.line === i) ? 'commented' : ''}`}
          >
            <ReactMarkdown remarkPlugins={[remarkGfm]}>{ln || ' '}</ReactMarkdown>
            {plan.comments
              .filter((c) => c.line === i)
              .map((c, ci) => (
                <div key={ci} className="ac-plan-comment">
                  <MessageSquarePlus size={10} /> {c.text}
                </div>
              ))}
            {pendingComment === i && (
              <div className="ac-plan-commentbox fade-in" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="choice-row">
                  ...
                  >
                    {busy ? 'Revising plan…' : 'Comment & regenerate'}
                  </button>
      ...
      {plan.status === 'pending_approval' || plan.status === 'revised' ? (
        <div className="choice-row">
          <button className="primary" disabled={busy} onClick={() => void approvePlan(task.id)}>
            Approve & Implement
          </button>
          <button className="danger" disabled={busy} onClick={() => void rejectPlan(task.id)}>
            Reject
          </button>
        ...
        <div className="choice-row">
          <span className="choice-made">Approved — implementation running below</span>
        </div>
      ) : null}
    </div>
  );
...
function FailureCard({
  ...
  task: { id: string; title: string; error?: string; model?: string; mode: string; prompt: string };
}) {
      ...
      <div className="ac-bubble">
        ...
        {m.pending && (
          <span
            ...
            aria-label={typed ? 'typing' : 'waiting'}
          >
            <span />
            <span />
            <span />
    ...
    >
      ...
      <div className="ac-header">
        <GlassButton
          ...
          onClick={newChat}
          title="New chat — clears the conversation"
        >
          <Plus size={13} />
        </GlassButton>
        <GlassButton
        ...
        {liveTask && (
          <span className="ac-live-badge">
            ...
            <span className="ac-live-title" title={liveTask.title}>
              {liveTask.title}
            </span>
            {liveTask.status === 'running' && (
              <button title="Pause" onClick={() => void taskAction(liveTask.id, 'pause')}>
                <Pause size={11} />
              </button>
            )}
            {liveTask.status === 'paused' && (
              <button title="Resume" onClick={() => void taskAction(liveTask.id, 'resume')}>
                <Play size={11} />
              </button>
            )}
            {(liveTask.status === 'running' || liveTask.status === 'paused') && (
              <button title="Cancel" onClick={() => void taskAction(liveTask.id, 'cancel')}>
                <X size={11} />
              </button>
            )}
            {(liveTask.status === 'failed' || liveTask.status === 'cancelled') && (
              <button
                title="Retry"
                onClick={() => void useStore.getState().startTask(liveTask.prompt, liveTask.mode)}
              >
                <RotateCw size={11} />
              </button>
            )}
          ...
          {sessions.map((s) => (
            ...
            >
              ...
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  useStore.getState().deleteChat(s.id);
                }}
              >
                <X size={11} />
              </button>
      ...
      <div className="ac-scroll" ref={scrollRef} onScroll={onScroll}>
        {empty && (
          <div className="empty-state" style={{ height: '100%' }}>
            ...
            <div style={{ fontSize: 11, textAlign: 'center' }}>
              ...
              <br />
              Tool calls and approvals appear right here in the flow.
              <br />
              <span style={{ color: 'var(--text-faint)' }}>
                Type /skill-id to invoke an enabled skill.
              </span>
            </div>
          </div>
        )}
        ...
        {chatMessages.map((m) => {
          ...
          return (
            <Bubble
              key={m.id}
              m={m}
              streamText={m.pending && liveTask ? streamText : undefined}
              live={m.pending && !!liveTask}
            />
          );
        })}
        ...
        {diffsPhase !== 'hidden' && (
          <div
            className={`ac-diffs-wrap ${diffsPhase === 'hiding' ? 'lifecycle-hide' : diffsPhase === 'finished' ? 'lifecycle-finished' : ''}`}
          >
            {diffs.map((d) => (
              <DiffArtifactCard key={d.id} diff={d} />
            ))}
          </div>
        )}
        ...
        {freshProposals.map((p) => (
          <div key={p.id} className="ac-prompt proposal fade-in">
            <div className="ac-prompt-head">
              <span className="warn">Proposed changes</span>
              <span className="ac-tool-param">
            ...
            <div className="choice-row">
              <button className="primary" onClick={() => void acceptProposal(p.id)}>
                Accept
              </button>
              <button className="danger" onClick={() => void rejectProposal(p.id)}>
                Reject
              </button>
              <button
                onClick={() => openTab({ kind: 'diff', title: 'Diff', path: `proposal:${p.id}` })}
              >
                View Diff
              </button>
            </div>
