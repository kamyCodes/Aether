...
interface TermInstance { id: string; title: string; cwd?: string; integrated?: boolean }
...
interface TermSuggestion { command: string; label: string }
...
function sanitizeTermChunk(data: string): string {
  ...
  if (lastEsc !== -1 && out.indexOf('\x07', lastEsc) === -1 && out.indexOf('\x1b\\', lastEsc) === -1) {
...
export function TerminalPanel() {
  ...
  const xtermRefs = useRef<Map<string, {
  ...
  useEffect(() => {
    let cancelled = false;
    if (!workspaceId) { setSuggestions([]); return; }
  ...
  useEffect(() => {
      ...
      const onMessage = (ev: MessageEvent) => {
        try {
          ...
          if (type === 'term:created') {
            ...
            const { id, title, cwd: newCwd } = payload as { id: string; title: string; cwd: string };
    ...
    if (existing && existing.readyState === WebSocket.OPEN) attach(existing);
      ...
      const t = setInterval(() => {
        const ws = getWs();
        if (ws && ws.readyState === WebSocket.OPEN) { attach(ws); clearInterval(t); }
      }, 300);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const term = new Terminal({
        ...
        fontFamily: "'JetBrains Mono', 'Cascadia Code', ui-monospace, 'SF Mono', Menlo, Consolas, monospace",
      ...
      const fitFn = () => { try { fit.fit(); } catch { /* hidden */ } };
          ...
          provideLinks: (lineNumber, cb) => {
            ...
            for (const re of [urlRe, pathRe]) {
              ...
              while ((m = re.exec(line))) {
                ...
                links.push({
                  text,
                  range: { start: { x: m.index + 1, y: lineNumber }, end: { x: m.index + text.length, y: lineNumber } },
                  activate: () => {
      ...
      if (buf) { buf.forEach((d) => term.write(d)); buffers.delete(activeId); }
    })();
  ...
  function runSuggestion(command: string) {
    ...
    if (target) { runIn(target.id); setActiveId(target.id); return; }
    ...
    <div className="term-panel">
      <div className="term-tabs">
        {terms.map((t) => (
          ...
          >
            ...
            {t.title}
            <button className="term-tab-close" onClick={(e) => killTerm(t.id, e)} title="Kill terminal">
              <X size={10} />
            </button>
          </span>
        ))}
        {/* Liquid Glass new-terminal button */}
        <GlassButton className="btn-glass glass term-new" onClick={createTerm} title="New terminal"><Plus size={13} /></GlassButton>
        <div className="term-tabs-spacer" />
        <button className={`term-search-btn ${searchOpen ? 'on' : ''}`} title="Search scrollback (Ctrl+F)" onClick={() => { setSearchOpen((v) => !v); setTimeout(() => searchRef.current?.focus(), 30); }}>
          <Search size={12} />
        </button>
        <span className="term-cwd" title={active?.cwd ?? cwd}>{active?.cwd ?? cwd}</span>
      </div>
      {searchOpen && (
        <div className="term-search-bar fade-in">
          <input
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === 'Escape') { setSearchOpen(false); activeEntry?.search?.clearDecorations?.(); }
            }}
            onChange={(e) => doSearch(e.target.value)}
          />
          <button title="Close" onClick={() => { setSearchOpen(false); activeEntry?.search?.clearDecorations?.(); }}><X size={11} /></button>
        </div>
      )}
      <div className="term-stage">
          ...
          <div className="term-idle">
            {suggestions.length > 0 ? (
              <>
                ...
                {suggestions.map((s) => (
                  <button key={s.command} className="term-idle-hint" onClick={() => runSuggestion(s.command)} title={`Run: ${s.command}`}>
                    <Play size={9} /> {s.label}
                  </button>
                ))}
              </>
            ) : (
              <span className="term-idle-hint" onClick={createTerm}>+ New terminal</span>
            )}
          </div>
        )}
        {terms.length > 0 && suggestions.length > 0 && (
          <div className="term-suggest-row" aria-label="Suggested commands">
            {suggestions.slice(0, 4).map((s) => (
              <button key={s.command} className="term-idle-hint" onClick={() => runSuggestion(s.command)} title={`Run in terminal: ${s.command}`}>
                <Play size={9} /> {s.label}
              </button>
            ))}
          </div>
        ...
        {suggestion && (
          ...
          >
            <Sparkles size={11} />
            <span>Ask agent about: <code>{suggestion.slice(0, 48)}</code></span>
            <X size={11} onClick={(e) => { e.stopPropagation(); setSuggestion(null); }} />
          </button>
        )}
      </div>
      ...
      <div className="term-status">
        {lastExit !== null && (
          lastExit === 0
            ? <span className="term-exit ok"><CircleCheck size={11} /> last command ok</span>
            : <span className="term-exit fail"><CircleX size={11} /> exit {lastExit}</span>
        )}
        ...
        <span className="term-status-hint">{active?.integrated === false ? 'basic shell' : 'integrated'}</span>
      </div>
    </div>
  );
}
