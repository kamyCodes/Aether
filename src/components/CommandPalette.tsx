...
export function CommandPalette({
  ...
  const commands: Cmd[] = useMemo(
    () => [
      ...
      {
        id: 'plan-task',
        label: 'Plan (no modifications)…',
        hint: 'agent',
        run: async () => {
          const prompt = await promptDialog({
            title: 'Plan without modifying',
      ...
      },
      {
        ...
        hint: 'view',
        run: () => store.openTab({ kind: 'search', title: 'Search' }),
      },
      {
        id: 'git',
        label: 'Open Git Panel',
        hint: 'view',
        run: () => store.openTab({ kind: 'git', title: 'Git' }),
      },
      {
        id: 'skills',
        label: 'Skills Manager',
        hint: 'view',
        run: () => store.openTab({ kind: 'skills', title: 'Skills' }),
      },
      {
        id: 'map',
        label: 'Codebase Map',
        hint: 'view',
        run: () => store.openTab({ kind: 'map', title: 'Codebase Map' }),
      },
      {
        id: 'checkpoint',
      ...
      {
        id: 'theme',
        label: 'Toggle Theme (dark / light / OLED)',
        hint: 'ui',
        run: () => {
          const s = store.settings;
          const next = { dark: 'light', light: 'oled', oled: 'dark' } as const;
          if (s)
            void store.saveSettings({ ...s, ui: { ...s.ui, theme: next[s.ui.theme] ?? 'dark' } });
        },
      },
      {
        id: 'open-file',
        label: 'Open File (system dialog)…',
        hint: 'file',
        run: () => openFileWithNativePicker(),
      },
      {
        id: 'add-ws',
    ...
    <div className="modal-overlay" onMouseDown={onClose}>
      <div className="modal" onMouseDown={(e) => e.stopPropagation()}>
        ...
        <div className="modal-results">
          {!isFileMode && filteredCmds.length > 0 && (
            <div className="palette-section">Commands</div>
          )}
          {filteredCmds.map((c) => (
            <div
              key={c.id}
              className={`result-item ${sel?.kind === 'cmd' && sel.id === c.id ? 'selected' : ''}`}
              onClick={() => {
                c.run();
                onClose();
              }}
          ...
          {filteredFiles.map((f) => (
            <div
              ...
              onClick={() => {
                void store.openFile(f);
                onClose();
              }}
            >
              <span className="file-ico">
                <FileIcon name={f.split('/').pop() ?? f} size={14} />
              </span>
              <span style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>{f}</span>
            </div>
          ))}
          {results.length === 0 && (
            <div className="result-item" style={{ color: 'var(--text-faint)' }}>
              No matches
            </div>
          )}
        </div>
