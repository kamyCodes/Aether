 ...
 */
export function TechIcon({ name, size = 20, color, className, title }: {
  name: string;
  ...
  return (
    <span
      className={className ? `tech-icon tech-icon-fallback ${className}` : 'tech-icon tech-icon-fallback'}
      title={title ?? name}
      style={{ width: size, height: size, display: 'inline-flex', flexShrink: 0, color: color ?? 'currentColor' }}
    >
      <svg width={size} height={size} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <path d="M6.2 8.2L7.8 9.8L6.2 11.4M9.8 8.2L8.2 9.8L9.8 11.4" stroke="currentColor" strokeWidth={1.2} strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    </span>
  );
}
