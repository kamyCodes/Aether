...
export function StatusBar() {
  ...
  return (
    <div className="status-line">
      <span className="status-item" title="Current workspace">{wsName ?? 'No workspace'}</span>
      {gitBranch ? (
        <span className="status-item status-branch" title="Current git branch (read-only display)">
          <GitBranch size={11} /> {gitBranch}
        </span>
      ) : (
        <span className="status-item status-branch status-norepo" title="No git repository connected in this workspace">
          <CircleOff size={11} /> no version control
        </span>
      )}
      <span className="status-sep">·</span>
      <span className="status-item" title={omniConnected ? 'AI endpoint reachable' : `AI endpoint unreachable${omniError ? `: ${omniError.slice(0, 80)}` : ''}`}>
        <span className={`conn-dot ${omniConnected ? 'ok' : 'bad'}`} /> Aether {omniConnected ? 'connected' : 'offline'}
      </span>
      <span className="status-sep">·</span>
      <span className="status-item" title={`WebSocket to the local server: ${wsStatus}`}>WS {wsStatus}</span>
      <span className="status-sep">·</span>
      <span className="status-item" title="Model used for chat and agent tasks">{chatModel || 'no model'}</span>
      {running > 0 && <><span className="status-sep">·</span>
        <span className="status-item" style={{ color: 'var(--warn)', display: 'inline-flex', alignItems: 'center', gap: 4 }} title={`${running} agent task${running > 1 ? 's' : ''} running`}>
      ...
      <button
        ...
        onClick={() => {
          ...
          const s = st.settings;
          if (s) void st.saveSettings({ ...s, ui: { ...s.ui, theme: THEME_CYCLE[s.ui.theme] ?? 'dark' } });
        }}
      >
