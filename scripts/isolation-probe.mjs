...
const run = (args, env, onOut, timeoutMs = 0) =>
  new Promise((resolve) => {
...
if (crashed.length)
  fail(
    `${crashed.length} writer process(es) exited non-zero:\n${crashed.map((c) => c.out).join('\n')}`,
  );
...
let final = null;
try {
  final = JSON.parse(fs.readFileSync(settingsFile, 'utf8'));
} catch (e) {
  fail(
    `settings.json is corrupt after concurrent writes: ${e instanceof Error ? e.message : String(e)}`,
  );
}
if (final) {
  if (
    /^worker-\d+-\d+$/.test(final.ui?.accent ?? '') &&
    typeof final.omni?.baseUrl === 'string' &&
    final.omni.baseUrl.startsWith('http')
  ) {
    write(
      `[part1] final file parses, contains one complete writer payload (accent=${final.ui.accent}), defaults intact — PASS`,
    );
  } else {
    fail(
      `final settings.json is not a coherent writer payload: ${JSON.stringify(final).slice(0, 200)}`,
    );
  }
}
...
write(
  '[part2] wrote: {"omni": null, "ui": {"theme": "light", "fontSize": 15}, "agent": {"autonomy": null}, "legacyUnknownTopLevel": true}',
);
...
let m = null;
try {
  m = JSON.parse(merged.out.trim().split('\n').pop() ?? '');
} catch {
  fail(`could not parse merge probe output:\n${merged.out}`);
}
if (m) {
  if (m.theme !== 'light' || m.fontSize !== 15)
    fail(`user data silently lost (expected light/15, got ${m.theme}/${m.fontSize})`);
  else if (!String(m.baseUrl).startsWith('http'))
    fail(`default baseUrl not filled in (got ${m.baseUrl})`);
  else if (m.autonomyMode !== 'review')
    fail(`null autonomy branch not healed (got ${m.autonomyMode})`);
  else
    write(
      `[part2] merge probe PASS — user values preserved (${m.theme}/${m.fontSize}), defaults filled (${m.baseUrl}, ${m.autonomyMode})`,
    );
}
...
if (sawListening)
  write('[part2b] server booted and bound a port against the old-schema settings — PASS');
else if (!sawListening && /Error|FATAL/i.test(boot.out))
  fail(`server crashed on old-schema settings:\n${boot.out.slice(0, 1200)}`);
else
  write(
    `[part2b] inconclusive (code=${boot.code}, killed=${boot.killed}) — full output below for review`,
  );
