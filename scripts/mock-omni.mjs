...
const chunk = (res, { content, tool_calls, finish, usage, model = MODEL }) => {
  sse(res, {
    id: 'mock', object: 'chat.completion.chunk', created: Date.now(), model,
    choices: [{ index: 0, delta: { ...(content ? { content } : {}), ...(tool_calls ? { tool_calls } : {}) }, finish_reason: finish ?? null }],
    ...(usage ? { usage } : {}),
  });
};
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/v1/chat/completions') {
    ...
    req.on('end', async () => {
      ...
      const sysText = parsed.messages.filter((m) => m.role === 'system').map((m) => (typeof m.content === 'string' ? m.content : '')).join(' ');
      ...
      if (sysText.includes('JSON array of 2-6 short step strings')) {
        chunk(res, { content: '["read existing utils files","create utils/greeting.ts with greet(name)","add a test for greet","run the test to verify"]' });
      ...
      if (lastMsg?.role === 'tool' && toolMsgCount >= 2) {
        ...
        chunk(res, { tool_calls: [{ index: 0, id: 'call_mem', type: 'function', function: { name: 'save_memory', arguments: JSON.stringify({ text: 'Pipeline verification ran; db-verify.md committed by the agent' }) } }] });
        ...
        chunk(res, { tool_calls: [{ index: 0, id: 'call_commit', type: 'function', function: { name: 'git_commit', arguments: JSON.stringify({ message: 'add db verification note' }) } }] });
