...
const EXTRA_CATEGORY = {
  ...
  '02-dependency-graph-impact-analyzer': ['debugging', ['impact-analysis', 'refactoring', 'dependencies', 'risk']],
  ...
  '08-xml-document-structure-validator': ['verification', ['xml', 'validation', 'documents', 'xsd']],
  '09-cross-repo-context-linker': ['import', ['cross-repo', 'context', 'multi-service', 'linking']],
  '10-performance-regression-sentinel': ['testing', ['performance', 'regression', 'benchmark', 'hot-paths']],
  '11-design-token-enforcer': ['ui', ['design-tokens', 'theming', 'styling', 'consistency']],
  '12-motion-interaction-reviewer': ['ui', ['interaction-states', 'motion', 'transitions', 'polish']],
  '13-accessibility-auditor': ['verification', ['accessibility', 'a11y', 'aria', 'contrast'], 'ui'],
  '14-component-api-critic': ['verification', ['components', 'api-design', 'conventions', 'review'], 'ui'],
  '15-visual-regression-diffing-agent': ['testing', ['visual-regression', 'breakpoints', 'diffing']],
  ...
  '18-design-to-code-diff-checker': ['design', ['design-to-code', 'fidelity', 'spec-matching'], 'ui'],
  '19-multi-theme-consistency-checker': ['ui', ['theming', 'variants', 'consistency']],
  '20-narrative-state-debugger': ['debugging', ['agentic-ui', 'state-desync', 'session-state'], 'ui'],
  ...
  '25-i18n-localization-completeness-checker': ['verification', ['i18n', 'l10n', 'translations', 'completeness']],
  ...
  '33-error-logging-consistency-enforcer': ['verification', ['logging', 'errors', 'consistency'], 'ops'],
  '34-commit-message-quality-skill': ['ops', ['git', 'commit-messages', 'conventions']],
  '35-full-app-from-prompt-builder': ['build', ['scaffolding', 'full-app', 'codegen', 'architecture']],
  ...
  '38-autonomous-root-cause-debugger': ['debugging', ['root-cause', 'instrumentation', 'hypotheses']],
  '39-live-architecture-decision-recorder': ['orchestration', ['adr', 'decisions', 'documentation']],
  ...
  '43-autonomous-adversarial-reviewer': ['verification', ['adversarial-review', 'self-review', 'diffs']],
  '44-prototype-to-production-hardener': ['build', ['hardening', 'validation', 'security', 'production'], 'ops'],
  ...
  '45-caveman-compressed-mode': ['output-style', ['terse', 'token-efficiency', 'prose', 'compression']],
};
...
function extractExtra(id, file) {
  ...
  return { id, name, file: relative(PACK, file).split(sep).join('/'), category, trigger, summary, tags: allTags, irregular: !trigger || !summary };
}
...
function extractCore(dirName, file) {
  ...
  const summary = fmDesc ?? firstSentence(md.split('\n').find((l) => l.trim() && !l.startsWith('#')) ?? '');
  ...
  return { id: dirName, name, file: relative(PACK, file).split(sep).join('/'), category, trigger, summary, tags: allTags, irregular: false };
}
...
for (const f of readdirSync(EXTRA).filter((f) => f.endsWith('.md') && f !== '00-README.md').sort()) {
  const id = f.replace(/\.md$/, '');
  const s = extractExtra(id, join(EXTRA, f));
  skills[s.id] = { name: s.name, file: s.file, category: s.category, trigger: s.trigger, summary: s.summary, tags: s.tags };
...
const CATEGORIES = ['ui', 'design', 'verification', 'testing', 'build', 'debugging', 'ops', 'orchestration', 'import', 'output-style'];
const LABELS = { ui: 'UI', design: 'Design', verification: 'Verification', testing: 'Testing', build: 'Build', debugging: 'Debugging', ops: 'Ops', orchestration: 'Orchestration', import: 'Import', 'output-style': 'Output Style' };
...
readme += `Machine-readable index: \`skills-manifest.json\`. Load skills programmatically with \`src/utils/skillsLoader.ts\` (filter by category or pick ids; concatenates full markdown into task context).\n\n`;
