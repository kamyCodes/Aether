...
ws.on('open', () => ws.send(JSON.stringify({ type: 'term:create', payload: { cwd: process.cwd() } })));
ws.on('message', (m) => {
  const { type, payload } = JSON.parse(m);
  if (type === 'term:created') {
    created = payload.id;
    integrated = payload.integrated;
    setTimeout(() => ws.send(JSON.stringify({ type: 'term:input', payload: { id: created, data: 'echo aether-e2e-ok\r' } })), 2000);
    setTimeout(() => ws.send(JSON.stringify({ type: 'term:input', payload: { id: created, data: 'node -e "process.exit(3)"\r' } })), 4000);
  }
  if (type === 'term:data') {
    raw += payload.data;
...
setTimeout(() => {
  ...
  console.log('structured commands received:', JSON.stringify(commands.map((c) => ({ cmd: c.command, code: c.exitCode, cwd: c.cwd?.split('\\').pop() }))));
  ...
  if (idx >= 0) console.log('context:', JSON.stringify(stripped.slice(Math.max(0, idx - 80), idx + 40)));
